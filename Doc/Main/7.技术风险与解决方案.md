...
**症状**: 日志显示数据已更新，但UI显示仍是旧值

**典型表现**:
- 日志: `[更新器] 缓存大小已更新: REAPER = 7406.62 MB`
- 日志: `[UI] 缓存大小已更新: A:\Code\REAPER = 7406.62 MB`
- **但UI表格仍显示**: `缓存大小: 0 B`

**根本原因**: WPF数据绑定属性缺少PropertyChanged通知机制
...
{
    public long CacheSize { get; set; }  // ❌ 无通知机制
    public string OriginalPath { get; set; }  // ❌ 无通知机制
    public string CachePath { get; set; }  // ❌ 无通知机制
}

// 问题：虽然 folder.CacheSize = newValue 执行成功
//       但WPF绑定系统没有收到PropertyChanged事件
//       导致UI不知道需要刷新显示
```
...
1. **检查属性实现**
   ```csharp
   // 查看属性定义，确认是否有OnPropertyChanged调用
   public long CacheSize
   {
       ...
       {
           _cacheSize = value;
           OnPropertyChanged(nameof(CacheSize));  // ✅ 必须有这行
       }
   }
   ```
...
2. **检查类继承**
   ```csharp
   // 确认类实现了INotifyPropertyChanged接口
...
3. **检查XAML绑定**
   ```xaml
   <!-- 确认绑定路径正确 -->
   <DataGridTextColumn Header="缓存大小"
                       Binding="{Binding CacheSizeFormatted}" />
   <!-- Path名称必须与属性名完全匹配 -->
   ```
...
public long CacheSize
{
    ...
    set
    {
        ...
        OnPropertyChanged(nameof(CacheSizeFormatted));  // 通知依赖属性
    }
}
```
...
**解决方法相同**：确保所有绑定属性都实现了PropertyChanged通知

**参考文档**:
- [3.项目规范与约定.md - INotifyPropertyChanged实现规范](./3.项目规范与约定.md#3-inotifypropertychanged实现规范)
...
问题: UI数据不更新
□ 检查属性是否有OnPropertyChanged调用
□ 检查类是否实现INotifyPropertyChanged
□ 检查XAML绑定路径
□ 添加调试日志验证事件触发
```
