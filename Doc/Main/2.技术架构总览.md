...
- [2.2 核心组件详解](#22-核心组件详解)
  ...
  - [2.2.9 CacheSizeUpdater](#229-cachesizeupdater)
  - [2.2.10 SingleInstanceManager](#2210-singleinstancemanager)
...
## 2.2.9 CacheSizeUpdater

**文件位置**: `CacheMax.GUI/Services/CacheSizeUpdater.cs`

### 职责

实现智能缓存大小更新机制，结合**防抖、强制更新和定时更新**三种策略，避免频繁磁盘I/O影响性能。

### 核心参数

```csharp
private const int DEBOUNCE_DELAY_MS = 8000;        // 防抖延迟：8秒
private const int MAX_DEBOUNCE_DELAY_MS = 45000;   // 最大防抖延迟：45秒
...
public void NotifyFileChange()
{
    ...
    _debounceTimer = new Timer(DebounceCallback, null, DEBOUNCE_DELAY_MS, Timeout.Infinite);
}
```

**适用场景**: 文件变化后等待8秒，避免频繁更新
...
**适用场景**: 大量文件持续变化时，确保至少每45秒更新一次

#### 3. 定时更新机制（45秒）

```csharp
// 无论是否有变化，每45秒主动更新一次
_periodicTimer = new Timer(_ => UpdateNow("定时更新"),
    null, PERIODIC_UPDATE_MS, PERIODIC_UPDATE_MS);
```
...
```csharp
// CacheManagerService中为每个加速文件夹创建独立updater
private readonly Dictionary<string, CacheSizeUpdater> _sizeUpdaters = new();
...
1. **延迟初始更新**: 等待2秒确保UI加载完成，避免时序问题
2. **大小写不敏感**: 路径匹配使用`StringComparison.OrdinalIgnoreCase`
3. **事件链路**: CacheSizeUpdater → CacheManagerService.StatsUpdated → MainWindow.OnCacheStatsUpdated
4. **线程安全**: 使用Timer确保更新操作的线程安全

---

## 2.2.10 SingleInstanceManager
