...
#define PROGNAME                        "passthrough-mod"
...
typedef struct
{
    HANDLE Handle;
    PVOID DirBuffer;
    BOOLEAN IsCache;  /* New: indicates if handle points to cache */
    WCHAR FileName[MAX_PATH];  /* Store file name for cache operations */
...
// Thread function for parallel write
static unsigned __stdcall WriteThreadFunc(void* param)
{
    WRITE_THREAD_CONTEXT *ctx = (WRITE_THREAD_CONTEXT*)param;
    OVERLAPPED Overlapped = { 0 };

    Overlapped.Offset = (DWORD)ctx->Offset;
    Overlapped.OffsetHigh = (DWORD)(ctx->Offset >> 32);

    if (WriteFile(ctx->Handle, ctx->Buffer, ctx->Length, ctx->BytesTransferred, &Overlapped))
        ctx->Result = STATUS_SUCCESS;
    else
        ctx->Result = FspNtStatusFromWin32(GetLastError());
...
static unsigned __stdcall StatsPrintThreadFunc(void* param)
{
    PTFS *Ptfs = (PTFS*)param;
    CACHE_STATS LastStats = { 0 };

    while (TRUE)
    {
        Sleep(5000);  // Print stats every 5 seconds

        CACHE_STATS CurrentStats = Ptfs->Stats;

        // Calculate rates
        UINT64 ReadOpsRate = CurrentStats.ReadOperations - LastStats.ReadOperations;
        UINT64 WriteOpsRate = CurrentStats.WriteOperations - LastStats.WriteOperations;
        UINT64 BytesReadRate = CurrentStats.BytesRead - LastStats.BytesRead;
        UINT64 BytesWrittenRate = CurrentStats.BytesWritten - LastStats.BytesWritten;

        // Calculate cache hit rate
        UINT64 TotalOps = CurrentStats.CacheHits + CurrentStats.CacheMisses;
        double HitRate = TotalOps > 0 ? (double)CurrentStats.CacheHits / TotalOps * 100.0 : 0.0;

        wprintf(L"[CACHE STATS] Hits: %llu, Misses: %llu, Hit Rate: %.1f%%, Read: %llu ops (%llu MB), Write: %llu ops (%llu MB)\n",
            CurrentStats.CacheHits, CurrentStats.CacheMisses, HitRate,
            ReadOpsRate, BytesReadRate / (1024 * 1024),
            WriteOpsRate, BytesWrittenRate / (1024 * 1024));
...
// Simple DAX implementation - no complex cache pool
    ...
    PVOID *PFileContext, FSP_FSCTL_FILE_INFO *FileInfo)
{
    ...
    // Also create file in cache if cache path exists
    if (Ptfs->CachePath != NULL && !(CreateOptions & FILE_DIRECTORY_FILE))
    {
        WCHAR CacheFullPath[FULLPATH_SIZE];
        if (ConcatPath(Ptfs, FileName, CacheFullPath))
        {
            // Replace source path with cache path
            wcscpy_s(CacheFullPath, FULLPATH_SIZE, Ptfs->CachePath);
            wcscat_s(CacheFullPath, FULLPATH_SIZE, FileName);

            // Create directories if needed
            WCHAR *LastSlash = wcsrchr(CacheFullPath, L'\\');
            if (LastSlash != NULL)
            {
                *LastSlash = L'\0';
                SHCreateDirectoryExW(NULL, CacheFullPath, NULL);
                *LastSlash = L'\\';
            }

            // Create cache file
            HANDLE CacheHandle = CreateFileW(CacheFullPath,
                GrantedAccess, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, &SecurityAttributes,
                CREATE_NEW, CreateFlags | FileAttributes, 0);
            if (INVALID_HANDLE_VALUE != CacheHandle)
            {
    ...
    wcscpy_s(FileContext->FileName, MAX_PATH, FileName);
    FileContext->IsCache = FALSE;
...
{
    PTFS *Ptfs = (PTFS *)FileSystem->UserContext;
    WCHAR SourcePath[FULLPATH_SIZE];
    ULONG CreateFlags;
    PTFS_FILE_CONTEXT *FileContext;
    ...
    /* Always open from source - caching happens at Read/Write level */
    if (!ConcatPath(Ptfs, FileName, SourcePath))
    {
        free(FileContext);
        return STATUS_OBJECT_NAME_INVALID;
    }

    FileContext->Handle = CreateFileW(SourcePath,
    ...
    PVOID FileContext0)
{
    ...
    if (FileContext->CacheMappedView != NULL)
    {
        UnmapViewOfFile(FileContext->CacheMappedView);
    }
    if (FileContext->CacheMapping != NULL)
    {
        CloseHandle(FileContext->CacheMapping);
    }
...
{
    PTFS *Ptfs = (PTFS *)FileSystem->UserContext;
    PTFS_FILE_CONTEXT *FileCtx = (PTFS_FILE_CONTEXT *)FileContext;
    HANDLE Handle = HandleFromContext(FileContext);
    ...
    *PBytesTransferred = 0;
    ...
    // Try to read from DAX-enabled cache using memory mapping (ZERO-COPY!)
    if (Ptfs->CachePath != NULL)
    {
            ...
            FileCtx->FileName))
        {
            ...
            {
                // Try to open and map cache file
                HANDLE CacheFileHandle = CreateFileW(CachePath,
                    GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                    OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

                if (CacheFileHandle != INVALID_HANDLE_VALUE)
                {
                    // Check if cache is up to date
                    FILETIME OriginalTime, CacheTime;
                    if (GetFileTime(Handle, NULL, NULL, &OriginalTime) &&
                        GetFileTime(CacheFileHandle, NULL, NULL, &CacheTime) &&
                        CompareFileTime(&CacheTime, &OriginalTime) >= 0)
                    {
                        // Cache is valid, create memory mapping for DAX zero-copy access
                        LARGE_INTEGER FileSize;
                        if (GetFileSizeEx(CacheFileHandle, &FileSize))
                        {
                            FileCtx->CacheMapping = CreateFileMapping(CacheFileHandle, NULL,
                                PAGE_READONLY, 0, 0, NULL);

                            if (FileCtx->CacheMapping != NULL)
                            {
                                FileCtx->CacheMappedView = MapViewOfFile(FileCtx->CacheMapping,
                                    FILE_MAP_READ, 0, 0, 0);

                                if (FileCtx->CacheMappedView != NULL)
                                {
                                    FileCtx->CacheFileSize = FileSize.QuadPart;
                                }
                            }
                        }
                    }
            ...
            // If we have a mapped cache, use ZERO-COPY read
            if (FileCtx->CacheMappedView != NULL && Offset + Length <= FileCtx->CacheFileSize)
            {
    ...
    // Fallback to traditional read if cache miss
    if (!ReadFromCache)
    {
        OVERLAPPED Overlapped = { 0 };
        ...
        BOOL ReadResult = ReadFile(Handle, Buffer, Length, PBytesTransferred, &Overlapped);
        Status = ReadResult ? STATUS_SUCCESS : FspNtStatusFromWin32(GetLastError());

        InterlockedIncrement64(&Ptfs->Stats.CacheMisses);

        // Consider creating cache for future reads
        if (STATUS_SUCCESS == Status && Ptfs->CachePath != NULL && *PBytesTransferred > 0)
        {
                ...
                FileCtx->FileName))
            {
                ...
                wcscpy_s(CacheDir, MAX_PATH, CachePath);
                WCHAR* LastSlash = wcsrchr(CacheDir, L'\\');
                if (LastSlash != NULL)
                {
                    *LastSlash = L'\0';
                    SHCreateDirectoryExW(NULL, CacheDir, NULL);
                }

                // Get original file path for copying
                WCHAR OriginalPath[MAX_PATH];
                if (ConcatPath(Ptfs, FileCtx->FileName, OriginalPath))
                {
                    // Asynchronously copy to DAX cache for future zero-copy access
                    CopyFileW(OriginalPath, CachePath, FALSE);
                }
            }
        }
    }
    ...
    InterlockedIncrement64(&Ptfs->Stats.ReadOperations);
    InterlockedAdd64(&Ptfs->Stats.BytesRead, *PBytesTransferred);
    ...
    PULONG PBytesTransferred, FSP_FSCTL_FILE_INFO *FileInfo)
{
    ...
    HANDLE CacheHandle = INVALID_HANDLE_VALUE;
    WRITE_THREAD_CONTEXT CacheWriteCtx = { 0 };
    HANDLE CacheThread = NULL;
    ULONG OriginalBytesWritten = 0, CacheBytesWritten = 0;

    // CRITICAL: READONLY ACCELERATION ONLY - NO CACHE WRITES!
    // All writes go directly to original file, never to cache
    // This prevents any delayed write risks and power loss data loss

    // Write ONLY to original file - NO CACHE UPDATES
    Overlapped.Offset = (DWORD)Offset;
    Overlapped.OffsetHigh = (DWORD)(Offset >> 32);

    if (!WriteFile(Handle, Buffer, Length, &OriginalBytesWritten, &Overlapped))
        Result = FspNtStatusFromWin32(GetLastError());

    *PBytesTransferred = OriginalBytesWritten;

    // After successful write to original file, invalidate cache file
    // so next read will create fresh cache copy
    ...
    {
        InterlockedIncrement64(&Ptfs->Stats.WriteOperations);
        InterlockedAdd64(&Ptfs->Stats.BytesWritten, OriginalBytesWritten);
        Result = GetFileInfoInternal(Handle, FileInfo);
    }
...
static NTSTATUS PtfsCreate(PWSTR Path, PWSTR CachePath, PWSTR VolumePrefix, PWSTR MountPoint, UINT32 DebugFlags,
    PTFS **PPtfs)
{
    ...
    *PPtfs = 0;
    ...
    // Simple DAX implementation without complex cache pool
    ...
    if (CachePath != NULL)
    {
        Length = GetFinalPathNameByHandleW(INVALID_HANDLE_VALUE, CachePath, 0, 0);
        if (0 == Length)
            Length = (ULONG)wcslen(CachePath);
        Length = (Length + 1) * sizeof(WCHAR);
        Ptfs->CachePath = malloc(Length);
        if (0 != Ptfs->CachePath)
        {
    ...
    if (CachePath != NULL)
    {
        Ptfs->StatsThread = (HANDLE)_beginthreadex(NULL, 0, StatsPrintThreadFunc, Ptfs, 0, NULL);
        if (Ptfs->StatsThread == NULL)
        {
            Result = FspNtStatusFromWin32(GetLastError());
            goto exit;
        }
...
static VOID PtfsDelete(PTFS *Ptfs)
{
    ...
    if (0 != Ptfs->StatsThread)
    {
        TerminateThread(Ptfs->StatsThread, 0);
        CloseHandle(Ptfs->StatsThread);
    }

    // Simple cleanup without cache pool
...
#define argtol(v)                       if (arge > ++argp) v = wcstol_deflt(*argp, v); else goto usage
    ...
    Result = PtfsCreate(PassThrough, CachePath, VolumePrefix, MountPoint, DebugFlags, &Ptfs);
    if (!NT_SUCCESS(Result))
    {
...
usage:
    static wchar_t usage[] = L""
        ...
        "    -p Directory        [directory to expose as pass through file system (source)]\n"
        "    -c CacheDirectory   [cache directory for accelerated read access]\n"
